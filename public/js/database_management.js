    ...
    if (doc.exists) {
      ...
      challengeRef.get().then(function(doc) {
        if (doc.exists) {
          ...
          firestore.collection("users").doc(userId).collection("assignedChallenges").doc(challengeID).set({
    ...
    var transaction = firestore.runTransaction(t => {
      return t.get(users.doc(groupOwnerUsername))
        .then(doc => {
          ...
          t.update(users.doc(groupOwnerUsername), {
            ownGroup: ownGroupArray
          });
        });
    }).then(result => {
      console.log('Transaction success!');
